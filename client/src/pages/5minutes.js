...
import getAllValues, { metamask, web3, SevenTOP, userAddress, changeUser, loadingBlockchain, connectMetaMask } from '../utils/connectBlockchain';
...
const FiveMinutes = () => {
	...
	const [winnersImgs, setWinnersImgs] = useState([]);
	...
	useEffect(() => {
		if (store.winners && winnersImgs.length < 2) {
			(async () => {
				for (const winner of store.winners) {
					const blob = await getProfilePic(winner.id);
					reader.onload = function () {
						setWinnersImgs((state) => [...state, this.result]);
					};
					reader.readAsDataURL(blob);
				}
			})();
		}
	}, [store.winners, winnersImgs.length]);
	...
	return (
		<div className="row game">
			{showResults ? (
				<div id="dataForSend">
					<p>
						Copy address of lottery:<i class="fa fa-times-circle copy-wallet" onClick={hide}></i>
					</p>
				...
				<div className="container">
					<div className="account">
						...
						<div className="elipse">
							<ImageUpload profilePic={userImg.img} token={auth.token} />
						</div>

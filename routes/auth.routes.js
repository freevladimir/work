...
router.post('/confirm-password', [check('password', 'Min length of password is 6 symbols').isLength({ min: 6 })], async (req, res) => {
...
router.post('/restore-password', [check('email', 'Incorrect email').isEmail()], async (req, res) => {
	const errors = validationResult(req);
	if (errors.isEmpty()) {
		...
		try {
			...
			if (errors.isEmpty()) {
				...
				if (user !== null) {
					...
					const url = `http://7top.org/new-password?token=${token}`;
					const isSent = await sendMail(email, req.app.get('transporter'), url, userName);
					if (isSent) {
						return res.json({ ok: true });
					}
					return res.status(400).json({ message: "We can't send you an email right now" });
				} else {
...
router.post(
	'/register',
	[check('email', 'Incorrect email').isEmail(), check('password', 'Min length of password is 6 symbols').isLength({ min: 6 })],
	async (req, res) => {
		try {
...
// /api/auth/login
router.post('/login', [check('password', 'Insert password').exists()], async (req, res) => {
	try {
		console.log('Body:', req.body);
		const errors = validationResult(req);
...
router.get('/friends', auth, async (req, res) => {
	try {
		...
		let refAbi = [
			{
				constant: false,
				inputs: [{ name: '_contr', type: 'address' }],
				name: 'addContract',
				outputs: [],
				payable: false,
			...
			{
				constant: false,
				inputs: [{ name: '_refCashbox', type: 'address' }],
				name: 'changeRefCashbox',
				outputs: [],
				payable: false,
				stateMutability: 'nonpayable',
				type: 'function',
			...
			{
				constant: false,
				inputs: [{ name: '_sevenTOP', type: 'address' }],
				name: 'setSevenTOP',
				outputs: [{ name: '', type: 'bool' }],
				payable: false,
				stateMutability: 'nonpayable',
			...
			{
				constant: false,
				inputs: [{ name: '_storageLimitLottery', type: 'address' }],
				name: 'setStorageLimitLottery',
				outputs: [{ name: '', type: 'bool' }],
				payable: false,
				stateMutability: 'nonpayable',
				type: 'function',
	...
	async (req, res) => {
		try {
			...
			for (let i = 0; i < members.length; i++) {
				...
				if (!user) {
					users.push({
						name: members[i][0].substr(0, 6) + '...' + members[i][0].substr(38, 4),
						time: members[i][1],
						sum: members[i][2],
						id: 'undefined',
					});
